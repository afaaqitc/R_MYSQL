...
hd<-
  hd |> group_by(customer_name) |> summarize(sales=sum(sales),.groups = "drop") |> 
...
names_from =country ,values_from = sales,values_fill = FALSE) |> adorn_totals("col") |> 
mutate(pct=percent(round( Total/sum(hd$sales),4))) |> adorn_totals("row")
...
summarize(sales=sum(sales),.groups = "drop")|>
pivot_wider(id_cols = c(emp,years),names_from = months,values_from = sales,values_fill = F) |> 
arrange(years) |> adorn_totals("col")|> 
...
lst<-
  list("Total_per_MY"=Totals_monthyears,
       "Cities"=top10_cities,"customers_my"=top10_customers,"Customers_countries"=top10_customers_countries,
       "items"=top10_items,
       "countries"=top5_countries,"countriesby_emp"= top5_countries_byEmp,
       "emp_sales"=emp_sales)
...
password=PG0KQpCYqEwX

dbname = if0_37540174_XXX
